...
def thread(topic_id):
    if request.method == "POST":
        ...
        # Get thread author's username
        author_username = db.execute("SELECT username FROM users WHERE id=?", author_id)[0]["username"]
        
        return render_template("thread.html", thread=thread, topic_id=topic_id, replies=replies, username=username, pfp=pfp, author_username=author_username )
